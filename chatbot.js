...
Để mình hỗ trợ bạn tốt nhất, bạn đang ở trong trường hợp nào dưới đây thế?`,
    ...
    welcomeOptions: [
      { text: "🛠️ Mình chuẩn bị khởi nghiệp, chưa rõ hướng đi.", next: "flow_khoi_nghiep" },
      { text: "⚠️ Mình đang vận hành doanh nghiệp nhưng gặp rối ren.", next: "flow_van_hanh" },
...
Bạn điền nhanh thông tin vào form đăng ký ở dưới, hệ thống sẽ gửi file PDF báo cáo mẫu qua Email hoặc Zalo cho bạn ngay lập tức nhé. Mình đang cuộn tới form...`
  ...
  function handleUserChoice(option) {
    ...
    setTimeout(() => {
      ...
      if (option.next === "flow_khoi_nghiep") {
        appendMessage("bot", `Vậy bạn thuộc nhóm rất phù hợp. 

Report sẽ giúp lọc trước ngành nên thử, mô hình nên chọn và điểm mù cần tránh trước khi bạn bỏ vốn.

Bạn muốn xem report gồm gì hay điền form trước?`);
        appendOptions([
          { text: "📖 Xem report gồm gì", next: "faq_answer", faqId: "q4" },
          { text: "💳 Điền form đăng ký", next: "do_checkout" },
          { text: "💰 Hỏi giá sản phẩm", next: "faq_answer", faqId: "q7" }
        ]);
      } else if (option.next === "flow_van_hanh") {
        appendMessage("bot", `Vậy cần soi lại không chỉ ngành, mà cả mô hình và cách vận hành.

Nhiều khi mình không sai vì chọn ngành dở, mà vì chọn sai cách chơi trong ngành đó.

Bạn muốn điền form để mô tả tình huống hiện tại không?`);
        appendOptions([
          { text: "💳 Có, gửi form cho tôi", next: "do_checkout" },
          { text: "❓ Tôi muốn hỏi thêm câu hỏi khác", next: "show_faqs" }
        ]);
      } else if (option.next === "show_faqs") {
  ...
  function processUserText(text) {
    ...
    if (cleanText.includes("phù hợp") || cleanText.includes("hợp với tôi") || cleanText.includes("hợp không") || cleanText.includes("hợp với mình") || cleanText.includes("hợp với em") || cleanText.includes("phù hợp không")) {
      appendMessage("bot", `Việc xem sản phẩm có phù hợp hay không phụ thuộc rất lớn vào giai đoạn và tình huống hiện tại của bạn.

DNA Kinh Doanh phù hợp nhất nếu bạn đang:
- Chuẩn bị khởi nghiệp (chưa rõ hướng đi, muốn tránh mất vốn sai chỗ).
- Đang vận hành doanh nghiệp nhưng bị rối ren, xung đột với cộng sự.
- Muốn hiểu rõ thế mạnh, mô hình vận hành và chu kỳ dồn lực của bản thân.

Bên mình cam kết: nếu bối cảnh của bạn không phù hợp để làm report, bên mình sẽ nói thẳng chứ không nhận tiền để làm cố. Bạn có thể điền form đăng ký thông tin bối cảnh để đội ngũ đánh giá trước nhé.`);
      setTimeout(showStandardNavigation, 600);
      return;
    }

    if (cleanText.includes("nghĩ thêm") || cleanText.includes("suy nghĩ thêm") || cleanText.includes("cân nhắc thêm")) {
      appendMessage("bot", `Được chứ bạn, bạn cứ suy nghĩ thêm nhé. Mấy quyết định liên quan đến kinh doanh không nên vội vàng hay mua chỉ vì một đoạn chat nghe hợp tai.

Nhưng để bạn suy nghĩ có dữ liệu thực tế hơn, mình gợi ý bạn điền form trước. Chưa cần mua ngay. Khi điền form, bạn sẽ tự nhìn lại rõ hơn:
- Bản thân đang thực sự rối ở đâu.
- Mình đang phân vân giữa những hướng đi nào.
- Mình sợ mất gì (vốn, thời gian) nếu chọn sai.

Bạn điền thử form bối cảnh trước ở dưới nhé.`);
      setTimeout(showStandardNavigation, 600);
      return;
    }
